...
contract("MultiSigWallet",accounts => {
    ...
    describe("executeTransaction",() => {
        ...
        it("should execute",async () => {
            ...
            const tx = await wallet.getTransaction(0) //return transaction object
            assert.equal(tx.executed,true)
        })
        //should fail if already executed
        it("should fail if already executed",async () => {
            await wallet.executeTransaction(0,{from: owners[0]}) //executed tx
            await expect(wallet.executeTransaction(0,{from:owners[0]})).to.be.rejected // we expect it to be rejected since it is executed before
        })
    })
